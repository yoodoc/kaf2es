...
public class KafkaStreamJobImplES implements KafkaStreamJob {
	...
	/**
	 * 
	 * @param esAddress   : elastic search의 주소
	 * @param clusterName  : elastic search cluster 이름
	 * @param index 
	 * @param type
	 * @param routingKey : null일 경우 routing key를 설정하지 않는다.
	 * @param bulkRequestSize : 한번에 보낼 es data의 수
	 * @param maxRequestIntervalSec : 최대 bulk insert 대기 시간
	 * @throws Exception
	 */
	public KafkaStreamJobImplES(String esAddress, String clusterName, String index, String type, String routingKey, int bulkRequestSize, int maxRequestIntervalSec)
			throws Exception {
	...
	public synchronized KafkaStreamJobResult excute(String data) {
		appendBulkRequestBuilder(this.indexName, this.typeName, data);
		insertingDataCount++;
		long currentTime = System.currentTimeMillis();
		if (insertingDataCount > bulkRequestSize || (currentTime - lastSendTime) > maxReqeustIntervalSec*1000) {
	...
	private boolean sendBulkRequest() {
		try {
			...
			BulkResponse bulkResponse = this.bulkRequestBuilder.execute().actionGet();
